...
class Model(object):
    ...
    def build(self):
        ...
        layer = self.fc_layers[0]
        tensor = tf.reshape(self.gcn_activations[-1], shape=(-1,FLAGS.node_num*FLAGS.output_gat))
        hidden = layer(tensor)
        self.fc_activatinos.append(hidden)

        for layer in self.fc_layers[1:]:
            hidden = layer(self.fc_activatinos[-1])
            self.fc_activatinos.append(hidden)

        self.outputs = self.fc_activatinos[-1]
    ...
    def _build(self):
        ...
        self.fc_layers.append(fc_layer(input_dim=FLAGS.output_gat*FLAGS.node_num, output_dim=FLAGS.hidden1_fc, placeholders=self.placeholders, 
                                       act=tf.nn.leaky_relu, dropout=True, name_='1'))
        
        self.fc_layers.append(fc_layer(input_dim=FLAGS.hidden1_fc, output_dim=self.output_dim, placeholders=self.placeholders, 
                                       act=lambda x:x, dropout=True, name_='2'))

    def _loss(self):
        for var in self.gat_layers[0].vars.values():
            self.loss += FLAGS.weight_decay*tf.nn.l2_loss(var)
        for var in self.fc_layers[0].vars.values():
            self.loss += FLAGS.weight_decay*tf.nn.l2_loss(var)
...
flags.DEFINE_integer('output_dim', 2, 'Number of output_dim')
...
flags.DEFINE_integer('hidden1_fc', 64, 'Number of units in hidden layer 1 of fc')
...
for epoch in tqdm(range(FLAGS.epochs)):  #FLAGS.epochs
...
y_pred = np.argmax(test_pre[0], axis=1)
...
fpr, tpr, thresholds = roc_curve(y_true,test_pre[0][:,1])
roc_auc = auc(fpr, tpr)
print("accuracy, sensitivity, specificity, fscore, balanced_acc, auc:", acc, sensitivity, specificity, fscore, balanced_acc, roc_auc)

results.append([acc, sensitivity, specificity, fscore, balanced_acc, roc_auc])
